...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.table(inventors.map(({ first, last }) => `${first} ${last}`));
    // Array.prototype.sort()
    // 3. Sort the inventors by birthdate, oldest to youngest
    console.table(inventors.sort((a, b) => a.year > b.year ? -1 : 1));
    // Array.prototype.reduce()
    // 4. How many years did all the inventors live?
    console.log(inventors.reduce((totalYear, { year, passed }) => totalYear + passed - year, 0));
    ...
    console.table(data.reduceRight((obj, item) => {
        if (!obj[item])
            obj[item] = 0;
        obj[item]++;
        return obj;
